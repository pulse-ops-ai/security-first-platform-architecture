...
- Each command is short and either:
  ...
  - Routes to a canonical skill in the architecture repo (`../security-first-platform-architecture/.agents/skills/<name>/SKILL.md`).
- Commands do not re-implement skills; they invoke them.

## Recommended consumer commands

The architecture repo ships these skills that are useful from a consumer context:

- `/repo-healthcheck` — runs `scripts/repo-healthcheck.sh` against this repo (consumer-repo mode). Copy the architecture repo's `.claude/commands/repo-healthcheck.md` into this directory and adjust paths as needed.
