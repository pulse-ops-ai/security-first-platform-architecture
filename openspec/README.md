...
- [`proposals/2026-05-28-v0.2.0-housekeeping/`](proposals/2026-05-28-v0.2.0-housekeeping/) — Tier 2, accepted (merged in PR #24), `v0.2.0` tagged at `25a3724`; pending archive in the next housekeeping PR. Bundled four housekeeping fixes: rewrote the standard's "deferred Phase C / *Extras → Edit Diagram Style*" wording to describe the swatch-and-copy library, recommended `--embed-svg-fonts false` in the `drawio` skill, and archived PR #22 + PR #23 proposals.
- [`proposals/2026-05-29-diagramming-conventions-v0.2.1-clarifications/`](proposals/2026-05-29-diagramming-conventions-v0.2.1-clarifications/) — Tier 2, in_review: patch-level clarifications to `standards/diagramming-conventions.md` surfaced by platform-edge's PR #5 diagram refresh. (B1) §Layer ribbons future-ribbon rule no longer colours the label text with the per-layer stroke (which fails the AA floor against its own paired fill) — uses `#222222` bold; §Common contrast failures updated to match. (B2) §Step-number badges gains a third anchor (full-width-ribbon left edge) + an edge-label collision rule. Additive; existing diagrams grandfathered; prerequisite for the v0.3.0 `validate-diagrams.sh` (D). Ships `v0.2.1` post-merge (or folds into v0.3.0 housekeeping).
- [`proposals/2026-05-29-diagramming-phase-c2-archetype-templates/`](proposals/2026-05-29-diagramming-phase-c2-archetype-templates/) — Tier 2, in_review (part of the `v0.3.0` set): ships `architecture/diagrams/templates/` with a starter template for all seven archetypes — 4 drawio (trust-zone/layer, deployment-topology, C4 System Context, C4 Container) + paired SVGs, and 3 Mermaid (trust-zone sequence, C4 Dynamic, decision-tree) — plus a pick-by-question `README.md`. Vendor-neutral `<placeholder>` skeletons that already carry the palette, iconography, contrast-safe labels, and footer stub. Stacked on the v0.2.1 clarifications branch. Additive; opt-in; no consumer forced.
- [`proposals/2026-05-29-diagramming-phase-d-validate-diagrams/`](proposals/2026-05-29-diagramming-phase-d-validate-diagrams/) — Tier 2, in_review (part of the `v0.3.0` set): ships `scripts/validate-diagrams.sh` + a pre-commit hook (the 20th) enforcing, per reference diagram, a paired SVG and a fresh `Last reviewed:` footer (hard fails), plus a WCAG-AA contrast lint (warning by default, `--strict` to fail). Auto-detects architecture vs consumer mode; excludes `templates/` and `styles/`. Flips the standard / INDEX / scripts-README references from "future" to "shipped." First finding: the canonical eight-layer reference predates v0.2.1 and trips 5 contrast warnings (grandfathered; refresh is a recommended follow-up). Stacked on C.2. Additive; no new rules (enforces existing MUSTs).
- [`proposals/2026-05-29-diagramming-phase-e-consumer-scaffold/`](proposals/2026-05-29-diagramming-phase-e-consumer-scaffold/) — Tier 2, in_review (completes the `v0.3.0` set): scaffolds `templates/consuming-repo/` with a starter `docs/diagrams/INDEX.md` + a `.gitignore` (folds the drawio-autosave pattern), and wires `validate-diagrams.sh` into the reusable `docs-healthcheck.yml` so consumer CI enforces paired-SVG + footer on their diagrams (full-scan mode; pre-v0.3.0 graceful skip). Stacked on D. Versioned workflow — existing `v0.2.0` consumers unaffected until they bump. Additive; opt-in.
