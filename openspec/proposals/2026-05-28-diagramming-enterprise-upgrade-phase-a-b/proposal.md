...
- **§Diagram archetypes table.** Seven archetypes spelled out: (i) trust-zone / layer architecture; (ii) deployment topology; (iii) C4 System Context (Level 1); (iv) C4 Container (Level 2); (v) trust-zone sequence (Mermaid `sequenceDiagram` with `(Zn)` participant notation); (vi) C4 Dynamic (Mermaid `C4Dynamic`, no zone notation); (vii) decision tree / state machine (Mermaid `flowchart` / `stateDiagram-v2`). Each row: question it answers, format, vocabulary. The trust-zone-sequence and C4-Dynamic split codifies the anti-mixing rule (trust-zone vocabulary in one, C4 vocabulary in the other; never both in one diagram).
